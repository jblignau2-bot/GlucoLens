/**
 * Shared Glucose / Weight tracker screen.
 *
 * Rendered by both the hidden "glucose" tab and the pushed /health-log route
 * (which shows a back button). Extracted here so the two routes don't
 * duplicate 800 lines of identical code.
 *
 * Dual functionality:
...
import { useGlucoseUnit } from "@/stores/profileStore";
import { LineChart } from "react-native-chart-kit";
import { Dimensions } from "react-native";
import { Plus, Droplets, Scale, TrendingDown, TrendingUp, Minus, Trash2, ArrowLeft } from "lucide-react-native";
...
/**
 * Context-aware status for a reading. Classification runs on the API's
 * canonical valueMmol; dangerous lows/highs override with the strongest red.
 */
function bglStatus(valueMmol: number, readingType?: string | null): { label: string; color: string; bg: string } {
  const band = classify(valueMmol, "mmol/L", normalizeContext(readingType));
  const danger = isDangerous(valueMmol, "mmol/L");
  const c = bandColors[band];
  if (danger.danger) {
    return { label: danger.kind === "low" ? "Very low" : "Very high", color: dangerColors.text, bg: dangerColors.bg };
  }
  return { label: c.label, color: c.text, bg: c.bg };
}

// ─── Reading context options ────────────────────────────────────────────────

const CONTEXT_OPTIONS: { value: ReadingContext; label: string }[] = [
...
function StatTile({ label, value, unit, trend, sub, accent }: { label: string; value: string; unit: string; trend?: "up" | "down" | "flat"; sub?: string; accent?: string }) {
  ...
  return (
    <View style={{
      ...
      <View style={{ flexDirection: "row", alignItems: "flex-end", gap: 4, marginTop: 6 }}>
        <Text style={{ fontSize: 22, fontWeight: "800", color: accent ?? colors.textPrimary }}>{value}</Text>
      ...
      {sub && (
        <Text style={{ fontSize: 10, color: colors.textMuted, marginTop: 4, lineHeight: 13 }}>
          {sub}
        </Text>
      )}
    </View>
...
function GlucoseRow({ entry, unit, onDelete }: { entry: { id: number; valueMmol: number; readingType?: string | null; loggedAt: string; notes?: string | null }; unit: GlucoseUnit; onDelete: (id: number) => void }) {
  const s = bglStatus(entry.valueMmol, entry.readingType);
  const displayValue = formatReading(convert(entry.valueMmol, "mmol/L", unit), unit);
  ...
  const handleLongPress = () => {
    ...
    Alert.alert(
      "Delete Reading",
      `Remove this ${displayValue} ${unit} reading?`,
      [
        { text: "Cancel", onPress: () => {} },
        {
          text: "Delete",
      ...
      <Pressable onLongPress={handleLongPress} style={{
        ...
        <View style={{ flex: 1 }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
            ...
            <Text style={{ fontSize: 13, color: colors.textSecondary }}>{unit}</Text>
          ...
          <Text style={{ fontSize: 12, color: colors.textSecondary, marginTop: 1 }}>
            {contextLabel(entry.readingType)} · {format(new Date(entry.loggedAt), "d MMM, h:mm a")}
          </Text>
          {entry.notes ? (
...
function WeightRow({ entry, onDelete }: { entry: { id: number; weightKg: number; loggedAt: string; notes?: string | null }; onDelete: (id: number) => void }) {
  const [showDelete, setShowDelete] = useState(false);
  const panX = useRef(new Animated.Value(0)).current;
  const panResponder = useRef(
    PanResponder.create({
    ...
    <Animated.View style={{ transform: [{ translateX: panX }], marginBottom: 8 }} {...panResponder.panHandlers}>
      <Pressable onLongPress={handleLongPress} style={{
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ fontSize: 12, color: colors.textSecondary, marginTop: 1 }}>
            {format(new Date(entry.loggedAt), "d MMM yyyy, h:mm a")}
          </Text>
          {entry.notes ? (
            <Text style={{ fontSize: 12, color: colors.textSecondary, fontStyle: "italic", marginTop: 2 }} numberOfLines={1}>
              {entry.notes}
...
function AddGlucoseModal({ visible, unit, onClose, onSave }: {
  visible: boolean;
  unit: GlucoseUnit;
  onClose: () => void;
  onSave: (data: { value: number; readingType: ReadingContext; notes?: string }) => void;
}) {
  const [value, setValue] = useState("");
  const [readingType, setReadingType] = useState<ReadingContext>("random");
  ...
  const submit = () => {
    ...
    if (isNaN(n) || n < bounds.min || n > bounds.max) {
      Alert.alert(
        "Invalid value",
        `Please enter a value between ${formatReading(bounds.min, unit)} and ${formatReading(bounds.max, unit)} ${unit}`
      );
      return;
    }
    onSave({ value: n, readingType, notes: notes.trim() || undefined });
    setValue(""); setNotes(""); setReadingType("random");
    onClose();
  };
          ...
          <View style={{
            ...
            <View>
              ...
              <TextInput
                ...
                placeholder={unit === "mmol/L" ? "e.g. 5.6" : "e.g. 100"}
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginHorizontal: -4 }}>
                <View style={{ flexDirection: "row", gap: 8, paddingHorizontal: 4 }}>
                  {CONTEXT_OPTIONS.map((t) => (
                    <Pressable
                      key={t.value}
                      onPress={() => { Haptics.selectionAsync(); setReadingType(t.value); }}
                      style={{
                        paddingHorizontal: 14,
                        paddingVertical: 8,
                        borderRadius: 20,
                        backgroundColor: readingType === t.value ? colors.primary : colors.cardAlt,
                        borderWidth: 1,
                        borderColor: readingType === t.value ? colors.primary : colors.border,
                      }}
                    >
                      <Text style={{
                        fontSize: 13,
                        fontWeight: "600",
                        color: readingType === t.value ? "#fff" : colors.textSecondary,
                      }}>
                        {t.label}
                      </Text>
                    </Pressable>
...
export function GlucoseScreen({ showBackButton = false }: { showBackButton?: boolean }) {
  ...
  // Chart data (glucose converted into the user's preferred unit)
  const glucosePoints = (glucoseLogs ?? []).slice(0, 20).reverse();
  const weightPoints  = (weightLogs  ?? []).slice(0, 20).reverse();
  const glucoseChartData = { labels: glucosePoints.map(() => ""), datasets: [{ data: glucosePoints.length > 0 ? glucosePoints.map(e => convert(e.valueMmol, "mmol/L", unit)) : [0] }] };
  const weightChartData  = { labels: weightPoints.map(() => ""),  datasets: [{ data: weightPoints.length > 0  ? weightPoints.map(e => e.weightKg)  : [0] }] };

  // ── Glucose stats: 7-day time-in-range, 7-day average, estimated A1c ──────
  const now = Date.now();
  const DAY = 24 * 60 * 60 * 1000;
  const last7Days = (glucoseLogs ?? []).filter((e) => now - new Date(e.loggedAt).getTime() <= 7 * DAY);
  const last30Days = (glucoseLogs ?? []).filter((e) => now - new Date(e.loggedAt).getTime() <= 30 * DAY);

  const inRangePct = last7Days.length > 0
    ? Math.round(
        (100 * last7Days.filter((e) => classify(e.valueMmol, "mmol/L", normalizeContext(e.readingType)) === "normal").length)
          / last7Days.length
      )
    : null;

  const avgGlucose = last7Days.length > 0
    ? last7Days.reduce((s, e) => s + e.valueMmol, 0) / last7Days.length
    : null;

  // eA1c (GMI) needs enough data to be meaningful: at least 10 readings
  // spanning at least 5 distinct days within the last 30 days.
  const distinctDays = new Set(last30Days.map((e) => new Date(e.loggedAt).toDateString())).size;
  const a1cEstimate = last30Days.length >= 10 && distinctDays >= 5
    ? estimateA1c(last30Days.reduce((s, e) => s + toMgdl(e.valueMmol, "mmol/L"), 0) / last30Days.length)
    : null;

  // ── Severe-value warning (non-blocking; reading is already being saved).
  // Deferred slightly so the alert isn't swallowed by the closing modal.
  const warnIfDangerous = (value: number) => {
    const d = isDangerous(value, unit);
    if (!d.danger) return;
    setTimeout(() => showDangerAlert(d.kind!), 450);
  };

  const showDangerAlert = (kind: "low" | "high") => {
    if (kind === "low") {
      Alert.alert(
        "Severely low glucose",
        "Severely low glucose. If you have symptoms, treat immediately (fast-acting sugar) and seek medical help."
      );
    } else {
      Alert.alert(
        "Severely high glucose",
        "Severely high glucose. Follow your care plan and contact your healthcare provider."
      );
    }
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
        {tracker === "glucose" ? (
          <>
            {/* Stats strip: time-in-range, 7-day average, estimated A1c */}
            <View style={{ flexDirection: "row", gap: 10, marginBottom: 10 }}>
              <StatTile
                label="In Range (7d)"
                value={inRangePct != null ? `${inRangePct}` : "—"}
                unit="%"
                accent={inRangePct != null ? (inRangePct >= 70 ? colors.safe : inRangePct >= 50 ? colors.moderate : colors.risky) : undefined}
              />
              <StatTile
                label="7-Day Avg"
                value={avgGlucose != null ? formatReading(convert(avgGlucose, "mmol/L", unit), unit) : "—"}
                unit={unit}
              />
            </View>
            <View style={{ flexDirection: "row", gap: 10, marginBottom: 16 }}>
              <StatTile
                label="Est. A1c (GMI)"
                value={a1cEstimate != null ? a1cEstimate.toFixed(1) : "—"}
                unit={a1cEstimate != null ? "%" : "more data needed"}
                sub={a1cEstimate != null
                  ? "Estimate only — not a lab value"
                  : "Needs 10+ readings on 5+ days in the last 30 days"}
              />
            </View>
